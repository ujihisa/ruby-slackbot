FROM rubylang/ruby:3.2.0-focal
...
ENV \
      APP_HOME=/app \
      BUNDLE_PATH=/vendor/bundle/3.2.0 \
      RUBYOPT=--yjit
...
RUN mkdir -p /usr/local/include/ruby-3.2.0/site_ruby && chown ubuntu /usr/local/include/ruby-3.2.0/site_ruby # dirty hack just for digest gem's bug
