FROM rubylang/ruby:3.2.1-focal
...
ENV \
      APP_HOME=/app \
      BUNDLE_PATH=/vendor/bundle/3.2.1 \
      RUBYOPT=--yjit
...
RUN mkdir -p /usr/local/include/ruby-3.2.1/site_ruby && chown ubuntu /usr/local/include/ruby-3.2.1/site_ruby # dirty hack just for digest gem's bug
